...
def merge_benchmark_and_publisher_data(df_benchmark_data, df_publisher_data):
    ...
    df_benchmark_data_copy['benchmark_timestamp'] = pd.to_datetime(df_benchmark_data_copy['benchmark_timestamp']).astype('datetime64[ns]')
    df_publisher_data_copy['publisher_timestamp'] = pd.to_datetime(df_publisher_data_copy['publisher_timestamp']).astype('datetime64[ns]')
    ...
    if dropped_rows > 0:
        print(f"Dropped {dropped_rows} rows ({dropped_percentage:.2%} of original) due to missing benchmark timestamp within 60s")
    ...
    if 'mode' in globals() and mode in ['us-equities', 'us-equities-pre', 'us-equities-post']:
        ...
        df_aligned_prices = pd.concat(market_data, ignore_index=True) if market_data else df_aligned_prices.iloc[:0]
        df_aligned_prices = df_aligned_prices.drop(['date'], axis=1)

        print(f"   Filtered to equity market hours (9:30:00-16:00:00 EST): {len(df_aligned_prices):,} observations remaining")

    # Calculate price difference
    df_aligned_prices['price_diff'] = df_aligned_prices['publisher_price'] - df_aligned_prices['benchmark_price']
    df_aligned_prices['price_diff_pct'] = (df_aligned_prices['price_diff'] / df_aligned_prices['benchmark_price']) * 100
    df_aligned_prices['abs_price_diff'] = df_aligned_prices['price_diff'].abs()
    df_aligned_prices['spread'] = df_aligned_prices['ask_price'] - df_aligned_prices['bid_price']
...
def calculate_publisher_metrics(df_aligned_prices):
    ...
    for publisher in df_aligned_prices['publisher_id'].unique():
        ...
        rmse = np.sqrt(mean_squared_error(pub_data['benchmark_price'], pub_data['publisher_price']))

        # Calculate nRMSE
        benchmark_range = pub_data['benchmark_price'].max() - pub_data['benchmark_price'].min()
        nrmse = rmse / benchmark_range if benchmark_range > 0 else np.nan

        rmse_over_spread = rmse / mean_spread
        mae = mean_absolute_error(pub_data['benchmark_price'], pub_data['publisher_price'])
        ...
        data_coverage = publisher_data_length / benchmark_data_length if benchmark_data_length > 0 else 0

        pass_fail = "pass" if (
            (nrmse < 0.01 or (nrmse < 0.05 and hit_rate > 98)) and
            (publisher_data_length >= 4800 or data_coverage >= 0.10) # 4800 for 1 update every 3 seconds over 4 hours, the smallest session length
        ) else "fail"
    ...
    metrics = sorted(metrics, key=lambda x: x['nrmse'] if not np.isnan(x['nrmse']) else float('-inf'), reverse=True)
...
def create_visualizations(df_aligned_prices, metrics_df):
    ...
    print(f"Plotting data from {plot_start} to {plot_end} ({len(plot_df):,} observations)")

    # Color palette for publishers
    colors = px.colors.qualitative.Plotly
    publisher_colors = {pub: colors[i % len(colors)]
    ...
    # Add Exchange price
    exchange_sample = plot_df.groupby('benchmark_timestamp')['benchmark_price'].first().reset_index()
    ...
    for publisher in plot_df['publisher_id'].unique():
        pub_data = plot_df[plot_df['publisher_id'] == publisher]
        fig_differences.add_trace(go.Scatter(
            x=pub_data['publisher_timestamp'],
    ...
    fig_box.update_layout(
        ...
        yaxis_title='Price Difference (%)',
        xaxis=dict(tickmode='array', tickvals=list(range(len(publishers))), ticktext=publishers),
        height=600,
        showlegend=False
    )

    # 5. Heatmap of publisher metrics (without legend)
    heatmap_metrics = ['mean_diff', 'std_diff', 'rmse', 'mae', 'hit_rate_0.1pct', 'mean_abs_z_score']
    ...
    fig_rmse.update_layout(
        ...
        height=max(400, len(metrics_df) * 25),  # Dynamic height based on number of publishers
    ...
    for i, publisher in enumerate(top_publishers):
        pub_data = plot_df[plot_df['publisher_id'] == publisher].sort_values('publisher_timestamp')
        if len(pub_data) > window:
            rolling_mean = pub_data['abs_price_diff'].rolling(window=window, min_periods=10).mean()
    ...
    for i, publisher in enumerate(top_publishers):
        pub_data = plot_df[plot_df['publisher_id'] == publisher].sort_values('publisher_timestamp')
        if len(pub_data) > window:
            rolling_std = pub_data['price_diff'].rolling(window=window, min_periods=10).std()
...
def print_summary_statistics(metrics_df, df_aligned_prices):
    ...
    print(f"Time period: {df_aligned_prices['publisher_timestamp'].min()} to {df_aligned_prices['publisher_timestamp'].max()}")
    print(f"Number of publishers: {len(metrics_df)}")
    print(f"\nMarket-wide statistics:")
    print(f"  Mean absolute difference: ${df_aligned_prices['abs_price_diff'].mean():.4f}")
    print(f"  Median absolute difference: ${df_aligned_prices['abs_price_diff'].median():.4f}")
    print(f"  95th percentile difference: ${df_aligned_prices['abs_price_diff'].quantile(0.95):.4f}")
...
def run_analysis(df_benchmark_data, df_publisher_data):
    ...
    df_aligned_prices = merge_benchmark_and_publisher_data(df_benchmark_data, df_publisher_data)
    print(f"   Successfully aligned {len(df_aligned_prices):,} price observations")

    if len(df_aligned_prices) == 0:
        print("ERROR: No aligned data found. Check if timestamps overlap between Exchange and Pyth data.")
        return None, None
    ...
    for publisher in df_aligned_prices['publisher_id'].unique():
        ...
        if std_diff > 0:
            df_aligned_prices.loc[mask, 'z_score'] = (pub_data['price_diff'] - mean_diff) / std_diff
        else:
...
def output_feed_readiness(metrics_df, output_path):
    ...
    if readiness_output_path.exists():
        ...
        done = df_to_write.drop_duplicates(subset=['feed_id', 'target_date'], keep='last')
    else:
        done = df_new_row
...
def parse_args():
    ...
    parser.add_argument("--feed-id", required=True, help="Pyth Lazer feed id, e.g. 1021")
    parser.add_argument("--date", required=True, help="UTC date YYYY-MM-DD, e.g. 2026-05-04")
...
def main():
    ...
    NOTEBOOK_OUTPUT_PATH = args.output_path if args.output_path else f"{repo_root}/dq_reports"
    ...
    try:
        ...
        if len(df_publisher_data) > 0:
            print(f"Queried Publisher data from ClickHouse: {len(df_publisher_data):,} rows")
        else:
    ...
    if not df_publisher_data.empty:
        ...
        df_publisher_data.drop_duplicates(subset=['publisher_timestamp', 'publisher_id'], keep='last', inplace=True)
        dropped_count = original_count - len(df_publisher_data)
        if dropped_count > 0:
            print(f"Removed {dropped_count} duplicate records (same publisher + timestamp)")
        ...
        if not pd.isna(exponent):
            ...
            df_publisher_data['publisher_price'] = df_publisher_data['publisher_price'] / divisor
    ...
    for channel in [1, 2, 3]:
        ...
        if len(df_feed_data_check) > 0:
            print(f"Using channel {channel} for Price Feed data ({len(df_feed_data_check):,} rows found)")
            break
    else:
    ...
    try:
        ...
        if len(df_feed_data) > 0:
            print(f"Queried Price Feed data from ClickHouse: {len(df_feed_data):,} rows")

            # Remove duplicates
            original_count = len(df_feed_data)
            df_feed_data.drop_duplicates(subset=['publisher_timestamp', 'publisher_id'], keep='last', inplace=True)
            ...
            if not pd.isna(exponent):
                divisor = 10 ** abs(exponent)
                df_feed_data['publisher_price'] = df_feed_data['publisher_price'] / divisor
                print(f"  Price adjusted by dividing by {divisor:,}")
            else:
                print("  Warning: Exponent is NaN, price not adjusted")

            # Append to df_publisher_data
            df_publisher_data = pd.concat([df_publisher_data, df_feed_data], ignore_index=True)
            print(f"Total publisher data rows after appending feed data: {len(df_publisher_data):,}")
        else:
    ...
    if mode == 'fx' or mode == 'metals':
        ...
        """
    elif mode == 'us-equities' or mode == 'us-equities-pre' or mode == 'us-equities-post':
        benchmark_query = f"""
            SELECT
                date_time as benchmark_timestamp,
                pyth_lazer_id as feed_id,
    ...
    try:
        ...
        if len(df_benchmark_data) > 0:
            ...
            df_benchmark_data.loc[df_benchmark_data['benchmark_price'].isna(), 'benchmark_price'] = (
            ...
            print(f"Benchmark data: {len(df_benchmark_data)} rows (filtered {filtered_count} rows with NaN values)")
        else:
    ...
    df_aligned_prices, metrics_df, figs = run_analysis(df_benchmark_data, df_publisher_data)
