...
async function init() {
  ...
  document.getElementById('targetDate').addEventListener('change', async (e) => {
...
async function loadPublishers() {
  try {
    ...
    select.innerHTML = publishers.map(p =>
      `<option value="${p.publisher_id}">${p.name || 'Publisher ' + p.publisher_id} (${p.pass_rate_pct?.toFixed(1) || 'N/A'}%)</option>`
    ).join('');
    ...
    document.getElementById('publisherId').innerHTML = '<option value="">Error loading publishers</option>';
  }
}
...
function updateSummaryCards(data) {
  ...
  if (data.benchmark.pass_rate_pct !== null) {
    ...
    passRateEl.parentElement.className = data.benchmark.pass_rate_pct >= 90
  ...
  if (data.benchmark.median_nrmse !== null) {
    ...
    nrmseEl.parentElement.className = data.benchmark.median_nrmse < 0.01
      ? 'summary-card summary-card--pass'
  ...
  if (data.uptime.overall_median_uptime_pct !== null) {
    uptimeEl.textContent = `${data.uptime.overall_median_uptime_pct.toFixed(2)}%`;
    uptimeEl.parentElement.className = data.uptime.overall_median_uptime_pct >= 99
      ? 'summary-card summary-card--pass'
  ...
  // Total Feeds
  document.getElementById('totalFeeds').textContent = data.benchmark.total_feeds || 0;
}
...
function updateAlerts(alerts) {
  ...
  if (!alerts.top_issues || alerts.top_issues.length === 0) {
    container.innerHTML = '<div class="empty-state"><div class="empty-state__message">No issues to address</div></div>';
    return;
  }

  container.innerHTML = alerts.top_issues.map(alert => `
    <div class="alert-item alert-item--${alert.severity}">
      <span class="alert-item__icon">${alert.severity === 'critical' ? '!' : 'i'}</span>
...
async function loadBenchmarkResults() {
  ...
  try {
    ...
    if (data.items.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="6" class="empty-state"><div class="empty-state__message">No benchmark results found</div></td></tr>';
    } else {
      tableBody.innerHTML = data.items.map(result => {
        ...
        return `
          <tr class="${rowClass}">
            ...
            <td>${result.hit_rate !== null ? result.hit_rate.toFixed(2) + '%' : 'N/A'}</td>
    ...
    tableBody.innerHTML = '<tr><td colspan="6" class="empty-state"><div class="empty-state__message">Error loading data</div></td></tr>';
  }
}
...
function updateBenchmarkPagination(data) {
  ...
  document.getElementById('benchmarkPrevBtn').disabled = state.benchmarkPage === 0;
...
async function loadUptimeResults() {
  ...
  try {
    ...
    if (results.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="6" class="empty-state"><div class="empty-state__message">No uptime data found</div></td></tr>';
    } else {
      tableBody.innerHTML = results.map(result => {
        const rowClass = result.uptime_pct >= 99
          ? 'data-table__row--pass'
